...
(function($) {
	...
	var exc_res = new Array();
	var leg_res = new Array();
	var lab_res = new Array();
		...
		for(var val in data){
			...
			$(el).append(out);
 			...
 			if(valueData instanceof Array){
				...
				for (i in valueData){
					...
					if(valueExc[i] == "")
					{
							o = "<div class='subBars"+el.id+"' id='"+exccounter+"' style='height:"+heig+"px; background-color: "+arr.colors[i]+"; cursor: pointer; left:"+wid*i+"px; color:"+arr.showValuesColor+"; font-size:"+fs+"px' >"+sv+"%"+"</div>";
					}
					else
					{
							o = "<div class='subBars"+el.id+"' id='"+exccounter+"' style='height:"+heig+"px; background-color: "+arr.colors[i]+"; cursor: pointer; left:"+wid*i+"px; color:"+arr.showValuesColor+"; font-size:"+fs+"px' >"+sv+"%"+"<br /><br /><img src='images/exception.png'> "+"</div>";
					}
			...
			//creating legend array from lbl if there is no legends param
 			...
 			if(arr.animate){
 				...
 				$('#graphFieldBar'+unique).css({'height': fieldHeight});
					...
					//Add functions here according to total number of bars to show the exceptions in the hovering effect. *******************************************
					//Since in my case total numbers of bars are 9, hence 0 to 8.
					
					
					jQuery.each(exc_res, function() {
							if(this[0] == "")
								this[0] = "<table class='leg_t' width='100%'><tr class='leg_th'><th colspan='2' id='exc_help'>Exceptions</th></tr><tr class='leg'><td>NO EXCEPTIONS</td></tr></table>";
							if(this[1] == "")
								this[1] = "<table class='leg_t' width='100%'><tr class='leg_th'><th colspan='2' id='exc_help'>Exceptions</th></tr><tr class='leg'><td>NO EXCEPTIONS</td></tr></table>";
							if(this[2] == "")
								this[2] = "<table class='leg_t' width='100%'><tr class='leg_th'><th colspan='2' id='exc_help'>Exceptions</th></tr><tr class='leg'><td>NO EXCEPTIONS</td></tr></table>";
					});
					
					$("#0").click(function(){
							$("#exceptions").html(exc_res[0][0]);
							$('#exceptions').show();
							$('#exc_help').show();
							...
							var str = "";
							var i = 0;
							for(i = 0; i < leg_res[0].length; i++)
							{
								str += "<tr class='leg'><td width='315px'>" + (i+1) + ".   " + leg_res[0][i] + "</td></tr>";
							}
							
							str = "<table class='leg_t' width='100%'><tr class='leg_th'><th class='leg_tf'>Legends for bars 1 to 3:</th></tr>" + str;
							str += "</table>";
							$("#legends").html("");
							$("#legends").append(str);
							$('#legends').show();
							...
							var l_str = "";
							var l_i = 0;
							for(l_i = 0; l_i < lab_res.length; l_i++)
							{
								l_str += "<tr class='leg'><td width='315px'>" + (l_i+1) + ".   " + lab_res[l_i] + "</td></tr>";
							}
							l_str = "<table class='leg_t' width='100%'><tr class='leg_th'><th class='leg_tf'>Excluded Labels:</th></tr>" + l_str;
							l_str += "</table>";
							$("#label-exc-div").html("");
							$("#label-exc-div").append(l_str);
							$('#label-exc-div').show();
							$('#label_exc_help').show();
					});
					$("#1").click(function() {
							$("#exceptions").html(exc_res[0][1]);
							$('#exceptions').show();
							$('#exc_help').show();
					...
					$("#3").click(function() {
							...
							var str = "";
							var i = 0;
							for(i = 0; i < leg_res[1].length; i++)
							{
								str += "<tr class='leg'><td width='315px'>" + (i+1) + ".   " + leg_res[1][i] + "</td></tr>";
							}
							str = "<table class='leg_t' width='100%'><tr class='leg_th'><th class='leg_tf'>Legends for bars 4 to 6:</th></tr>" + str;
							str += "</table>";
							$("#legends").html("");
							$("#legends").append(str);
							$('#legends').show();
					...
					$("#6").click(function() {
							...
							var str = "";
							var i = 0;
							for(i = 0; i < leg_res[2].length; i++)
							{
								str += "<tr class='leg'><td width='315px'>" + (i+1) + ".   " + leg_res[2][i] + "</td></tr>";
							}
							str = "<table class='leg_t' width='100%'><tr class='leg_th'><th class='leg_tf'>Legends for bars 6 to 9:</th></tr>" + str;
							str += "</table>";
							$("#legends").html("");
							$("#legends").append(str);
							$('#legends').show();
 		...
 		if(arr.legend){
			$(el).append("<div id='legendHolder"+unique+"'></div>");
	 		$('#legendHolder'+unique).css({ 'width': legendWidth, 'text-align' : 'left', 'position' : 'absolute', 'top' : '330px', 'left' : '30px'});
